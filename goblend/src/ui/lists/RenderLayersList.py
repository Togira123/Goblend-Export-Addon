...
def layer_items(_self, _context):
    if len(utils.render_layers_enum_cache) == 0:
        ...
        for layer in config["visuals"]["render_layers"]:
            utils.render_layers_enum_cache.append(
                (str(layer["bit"]), layer["display_name"], "Bit " + str(layer["bit"]))
            )
...
class LIST_OT_RemoveItemFromRenderLayersList(bpy.types.Operator):
    bl_idname = "render_layers_list.remove_item"
